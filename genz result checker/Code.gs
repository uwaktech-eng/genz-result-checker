...
function validateAction_(action) {
  var allowed = {
    ...
    validateSession: true, getAdminBootstrap: true, saveSettings: true, saveStudent: true, setStudentState: true,
...
function normalizeImageUrl_(url) {
  ...
  if (!url) return '';
  var match = url.match(/\/file\/d\/([a-zA-Z0-9_-]+)/i) || url.match(/[?&]id=([a-zA-Z0-9_-]+)/i);
